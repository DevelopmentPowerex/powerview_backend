...
from .protocols.auxiliar_info import PARAMETER_TRANSLATION,PREMADE_ORDERS, PARAMETERS_FOR_EVENTS, PARAMETERS_FOR_REPORT, PARAMETERS_CHART_ORDER
...
async def fetch_report_data(client_name:str,
    ...
    async with httpx.AsyncClient(timeout=settings.gateway_timeout) as client:
        ...
        circuits=await get_project_circuits(client_name,project_name ,client) #DATOS DEL PROYECTO
        ...
        measurements, extreme_values=await extract_readings(meter_list,start_date,end_date,client) #EXTRAER MEDICIONES PARA EL REPORTE
        logger.debug(extreme_values)
        if not (measurements):
            logger.error('Error while getting the readings information')

        event_list=await extract_events(meter_list,start_date,end_date,client) #EXTRAER EL REGISTRO DE EVENTOS 
